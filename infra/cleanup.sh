#!/usr/bin/env bash
# =============================================================================
#  Tear down Option A (APIM managed-identity swap).
#  ---------------------------------------------------------------------------
#  Deletes the whole resource group (APIM Basic v2, the Foundry account +
#  deployments + project, App Insights), then PURGES the soft-deleted Cognitive
#  Services account and APIM so their names free up immediately.
#
#  Does NOT touch the shared `copilot-users` group or `copilotuser` test user.
#  When APIM is deleted, its system-assigned managed identity (and the Foundry
#  RBAC + Microsoft Graph app-role assignments granted to it) are removed with
#  it; any briefly-orphaned RBAC assignment is garbage-collected by Azure.
#
#  Usage:
...
RG="${RG:-rg-copilot-foundry-poc}"           # Option A resource group
SVC="${SVC:-apim-copilot-poc}"               # APIM instance (for soft-delete purge)
FOUNDRY="${FOUNDRY:-cog-copilot-poc}"        # Cognitive Services account (for purge)
